...
import 'package:locationpicker/geolocator/platformInterface/geolocator_platform_interface.dart';
...
class GeolocatorAndroid extends GeolocatorPlatform {
  ...
  @override
  Future<bool> isLocationServiceEnabled() async => _methodChannel.invokeMethod<bool>('isLocationServiceEnabled').then((value) => value ?? false);
  ...
  Stream<ServiceStatus> getServiceStatusStream() {
    ...
    _serviceStatusStream = serviceStatusStream.map((dynamic element) => ServiceStatus.values[element as int]).handleError((error) {
  ...
  Stream<Position> getPositionStream({
    ...
    _positionStream = positionStream.map<Position>((dynamic element) => Position.fromMap(element.cast<String, dynamic>())).handleError(
      (error) {
        if (error is PlatformException) {
          error = _handlePlatformException(error);
        }
  ...
  @override
  Future<bool> openAppSettings() async => _methodChannel.invokeMethod<bool>('openAppSettings').then((value) => value ?? false);

  @override
  Future<bool> openLocationSettings() async => _methodChannel.invokeMethod<bool>('openLocationSettings').then((value) => value ?? false);
  ...
  @override
  Future<String> getLocationOnChoice() async {
    try {
      // ignore: omit_local_variable_types
      String loc = await _methodChannel.invokeMethod('locationChoice');
      return loc;
    } catch (e) {
      return 'error';
    }
